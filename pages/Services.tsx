        ...
        <div className="relative z-20 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl">
            ...
            <div className="flex gap-4">
              <Link to="/contact" className="h-12 flex items-center justify-center rounded-lg bg-primary px-6 text-sm font-bold text-white transition-colors hover:bg-blue-600">
                Get a Quote
              </Link>
            </div>
          </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
            {services.map((service, idx) => (
              <div key={idx} className="group relative flex flex-col overflow-hidden rounded-xl bg-white dark:bg-slate-800 shadow-sm transition-all hover:-translate-y-1 hover:shadow-lg border border-slate-200 dark:border-slate-700">
                <div className="h-48 w-full overflow-hidden">
                  <div className="h-full w-full bg-cover bg-center transition-transform duration-500 group-hover:scale-105" style={{ backgroundImage: `url("${service.img}")` }}></div>
                </div>
                <div className="flex flex-1 flex-col p-6">
                  ...
                  <p className="mb-6 text-base text-slate-600 dark:text-slate-400">{service.desc}</p>
                  <div className="mt-auto flex flex-wrap gap-2 mb-6">
                    {service.tags.map(tag => (
                      <span key={tag} className="inline-flex items-center rounded-full bg-slate-100 dark:bg-slate-700 px-2.5 py-0.5 text-xs font-medium text-slate-800 dark:text-slate-300">
                        {tag}
                      </span>
                    ))}
                  </div>
                  <Link to="/contact" className="group/link flex items-center text-sm font-bold text-primary">
                    Get Started <span className="material-symbols-outlined ml-1 text-lg transition-transform group-hover/link:translate-x-1">arrow_forward</span>
                  </Link>
                </div>
              </div>
            ))}
